...
class ProjectState extends State<Project> {
  ...
  private updateListeners() {
    for (const listenerFn of this.listeners) {
      listenerFn(this.projects.slice()); //.slice returns a copy (not a reference)
    }
  }
}
...
interface Validatable {
  ...
  maxLength?: number;
  min?: number;
  max?: number; // ? mean optional (number | undefined)
}
...
// abstract class is used for inharitance (and can't be instantiated!)
